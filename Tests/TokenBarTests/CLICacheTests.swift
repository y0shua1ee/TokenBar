...
struct CLICacheTests {
    ...
    func `cache clear parses cookies provider flags`() throws {
        let parser = CommandParser(signature: TokenBarCLI._cacheSignatureForTesting())
    ...
    func `provider scope is rejected for cost clearing`() {
        #expect(TokenBarCLI.cacheClearProviderScopeError(rawProvider: nil, clearCost: true) == nil)
        #expect(TokenBarCLI.cacheClearProviderScopeError(rawProvider: "claude", clearCost: false) == nil)
        #expect(TokenBarCLI.cacheClearProviderScopeError(rawProvider: "claude", clearCost: true)?
            .contains("--provider only scopes cookie caches") == true)
    }

    @Test
    func `cache help documents provider as cookie scoped`() {
        let help = TokenBarCLI.cacheHelp(version: "0.0.0")
