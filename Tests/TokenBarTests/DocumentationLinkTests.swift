...
struct DocumentationLinkTests {
    ...
    func `local documentation paths normalize safely`() throws {
        let root = URL(filePath: "/tmp/TokenBar-documentation-links", directoryHint: .isDirectory)

        let target = try Self.localDocURL(
            for: "./docs/guide%20name.md?mode=print#topic",
            repositoryRoot: root)
        #expect(target.path == "/tmp/TokenBar-documentation-links/docs/guide name.md")
