...
struct KeychainPromptSafetyAuditTests {
    ...
    func `live TTY integration tests are opt in`() throws {
        let ttyTests = try Self.readRepoFile("Tests/TokenBarTests/TTYIntegrationTests.swift")
    ...
    func `tests do not call SecItemCopyMatching except no UI query coverage`() throws {
        let offenders = try Self.swiftTestFiles().filter { file in
            ...
            return text.contains("SecItemCopyMatching")
                && !file.path.hasSuffix("Tests/TokenBarTests/KeychainNoUIQueryTests.swift")
                && !file.path.hasSuffix("Tests/TokenBarTests/KeychainPromptSafetyAuditTests.swift")
        }
    ...
    private static func swiftTestFiles(excludingSelf: Bool = false) throws -> [URL] {
        let testsRoot = self.repoRoot().appendingPathComponent("Tests/TokenBarTests", isDirectory: true)
        ...
        for case let file as URL in enumerator where file.pathExtension == "swift" {
            ...
            if values.isRegularFile == true {
                if excludingSelf, file.path.hasSuffix("Tests/TokenBarTests/KeychainPromptSafetyAuditTests.swift") {
                    continue
                }
                files.append(file)
            }
