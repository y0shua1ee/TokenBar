...
struct TokenAccountEnvironmentPrecedenceTests {
    ...
    func `codex CLI ignores relative profile homes`() throws {
        ...
        let managedStoreRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("codex-relative-profile-store-\(UUID().uuidString)", isDirectory: true)
        let managedStoreURL = managedStoreRoot.appendingPathComponent("managed-codex-accounts.json", isDirectory: false)
        defer { try? FileManager.default.removeItem(at: managedStoreRoot) }
        let context = try TokenAccountCLIContext(
            selection: TokenAccountCLISelection(label: nil, index: nil, allAccounts: false),
            config: config,
            verbose: false,
