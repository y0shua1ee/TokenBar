...
struct CLIServeRouterTests {
    ...
    func `serve numeric options reject malformed values`() {
        ...
        #expect(TokenBarCLI.decodeServeRequestTimeout(from: ParsedValues(
            positional: [],
    ...
    func `serve config snapshot reflects provider changes`() throws {
        ...
        let firstSnapshot = try TokenBarCLI.loadServeConfigSnapshot(configStore: store)
        ...
        let secondSnapshot = try TokenBarCLI.loadServeConfigSnapshot(configStore: store)
        ...
        #expect(
            TokenBarCLI.serveCacheKey(kind: "usage", provider: nil, configToken: firstSnapshot.cacheToken) !=
                TokenBarCLI.serveCacheKey(kind: "usage", provider: nil, configToken: secondSnapshot.cacheToken))
    }
    ...
    func `serve cache uses stable Codex account identities`() {
        ...
        let firstKey = TokenBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: firstProjection)
        let reshapedKey = TokenBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: reshapedProjection)
        let replacementKey = TokenBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: replacement)
        let workspacePeerKey = TokenBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: workspacePeer)
        let storedBeforeKey = TokenBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
            codexVisibleAccount: storedBeforeRefresh)
        let storedAfterKey = TokenBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
        ...
        #expect(TokenBarCLI.usageCacheAccountKey(
            provider: .codex,
            account: nil,
    ...
    func `serve cache coalesces concurrent cache misses`() async {
        ...
        let responses = await withTaskGroup(of: CLILocalHTTPResponse.self) { group -> [CLILocalHTTPResponse] in
            for _ in 0..<5 {
                group.addTask {
                    await TokenBarCLI.cachedServeResponse(
                        key: "usage:",
                        cache: cache,
                        refreshInterval: 60,
                        requestTimeout: 1)
    ...
    func `serve cache prunes expired config token entries`() async throws {
        ...
        _ = await TokenBarCLI.cachedServeResponse(
    ...
    func `serve cache does not cache timeouts and recovers on next success`() async {
        ...
        let timeout = await TokenBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 0.01)
        ...
        let success = await TokenBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
        ...
        let cached = await TokenBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 60,
            requestTimeout: 1)
    ...
    func `serve cache serves last good payload when refresh fails`() async {
        ...
        let first = await TokenBarCLI.cachedServeResponse(
        ...
        let failed = await TokenBarCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
        ...
        let recovered = await TokenBarCLI.cachedServeResponse(
            key: "usage:antigravity",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `serve cache replaces only failed provider account rows`() async throws {
        ...
        let refreshed = await TokenBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 1)
    ...
    func `serve cache fails closed on timeout after merged rows`() async {
        ...
        let timedOut = await TokenBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0.05,
            requestTimeout: 0.01)
    ...
    func `serve stale ttl is bounded and disabled without caching`() {
        #expect(TokenBarCLI.serveStaleTTL(refreshInterval: 0) == 0)
        #expect(TokenBarCLI.serveStaleTTL(refreshInterval: 1) == 300)
        #expect(TokenBarCLI.serveStaleTTL(refreshInterval: 60) == 600)
        #expect(TokenBarCLI.serveStaleTTL(refreshInterval: 1800) == 3600)
        #expect(TokenBarCLI.serveStaleTTL(refreshInterval: 86401) == 3600)
        #expect(TokenBarCLI.serveStaleTTL(refreshInterval: .infinity) == 3600)
    }
    ...
    func `serve helper idle window outlives the refresh cadence`() {
        #expect(TokenBarCLI.serveCLISessionIdleWindow(refreshInterval: 0) == 180)
        #expect(TokenBarCLI.serveCLISessionIdleWindow(refreshInterval: 60) == 180)
    ...
    func `serve request timeout zero disables the deadline`() async {
        ...
        let response = await TokenBarCLI.cachedServeResponse(
            key: "usage:",
            cache: cache,
            refreshInterval: 0,
            requestTimeout: 0)
