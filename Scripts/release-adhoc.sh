...
This path intentionally does not notarize, generate Sparkle appcast entries, or require
...
Options:
  ...
  --skip-checks      Skip make check and make test. Use only after those gates already passed.
...
package_dsym() {
  ...
  for index in "${!ARCH_LIST[@]}"; do
    ...
    arch_dsym=$(codexbar_require_dsym_dwarf_for_arch "${dsym_paths[$index]}" "$APP_NAME" "$arch")
  ...
  [[ -d "$dsym_path" ]] || die "Missing dSYM at: $dsym_path"
...
verify_adhoc_bundle() {
  local feed_url auto_checks signature
  ...
  [[ -z "$feed_url" ]] || die "Ad-hoc bundle must not include SUFeedURL; got: $feed_url"

  auto_checks=$(/usr/libexec/PlistBuddy -c 'Print :SUEnableAutomaticChecks' "$APP_BUNDLE/Contents/Info.plist" 2>/dev/null || true)
  [[ "$auto_checks" == "false" ]] || die "Ad-hoc bundle must disable Sparkle automatic checks; got: ${auto_checks:-missing}"
}
...
publish_release() {
  ...
  [[ "$current_branch" == "$release_branch" ]] || die "Release must run on $release_branch; current branch is ${current_branch:-detached}"
  ...
  gh repo view "$REPO" --json nameWithOwner --jq .nameWithOwner >/dev/null || die "Could not reach GitHub repo: $REPO"
...
require_bin git python3 swift lipo codesign xattr
