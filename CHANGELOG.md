...
- CLI server: reload provider config for every usage and cost request, invalidate config-dependent cache entries, and prune expired config variants without restarting `tokenbar serve`. Thanks @enieuwy!
...
- Claude: remove transient ClaudeProbe session artifacts after CLI usage polls so background refreshes no longer fill Claude Code project history with TokenBar `/usage` sessions (#1301). Thanks @LPFchan and @matthewod11-stack!
...
### Fixed
- Menu bar: stop forcing a private preferred-position value for fresh status items; suspicious stored positions are now cleared so AppKit can place TokenBar normally on macOS 26 / 5K displays (#1267). Thanks @AdrianSimionov, @kirocop, and @Yuxin-Qiao!
...
### Added
- QA: document the live TokenBar e2e flow and add a redacted provider-matrix helper for packaged CLI smoke tests.
...
### Fixed
- Claude: keep Claude CLI-owned OAuth refresh tokens delegated to Claude Code when CLI storage is present, preventing TokenBar from consuming rotating refresh tokens and forcing re-login (#1161, #1239). Thanks @RajvardhanPatil07!
...
### Changed
- Docs: update the Homebrew install command to use the official `tokenbar` cask now that it supports Intel Macs (#1189). Thanks @SSakutaro!
...
### Fixed
- CLI: bound `tokenbar serve` requests with a configurable timeout and coalesce concurrent cache misses so hung `/usage` callers no longer stampede provider refreshes (#1208). Thanks @enieuwy!
...
- Build script: make TokenBar-owned ad-hoc keychain cleanup opt-in with `--clear-adhoc-keychain`, and extend the explicit reset path to clear both `com.steipete.TokenBar` and `com.steipete.tokenbar.cache`. Thanks @magnaprog!
...
- Debug replay toggle (`defaults write com.steipete.tokenbar debugMenuEnabled -bool YES`) to view every pattern.
