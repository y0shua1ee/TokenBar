...
`Sources/CodexBar/KeychainMigration.swift` migrates legacy `com.steipete.TokenBar` items into the current
`com.y0shua1ee.TokenBar` service (for example `claude-cookie`) using `AfterFirstUnlockThisDeviceOnly`.
