...
TokenBar can use either a DeepSeek API key or a signed-in DeepSeek Platform session for the remaining credit balance.
Detailed cost and token usage comes from the Platform session; an API key cannot authenticate the private dashboard
endpoints.

## Data sources

1. **Optional API key** supplied via `DEEPSEEK_API_KEY` / `DEEPSEEK_KEY`, or selected from DeepSeek token accounts in `~/.tokenbar/config.json`.
...
TokenBar resolves the Platform `userToken` in this order:
...
TokenBar checks every Chrome profile containing a parseable `userToken` against DeepSeek. Rejected or expired
sessions are omitted. Settings shows a **Chrome profile** picker containing only valid sessions. With no API key, one
valid session is selected automatically; with multiple valid sessions, TokenBar reuses the saved choice or asks once.
When an API key supplies the balance, a new or changed API credential requires an explicit session selection before
website usage is combined with it. TokenBar persists a stable browser/profile identifier, not an absolute home-directory path, and
...
If the selected session expires, TokenBar asks before switching to another valid profile.
...
- The amount and cost requests run concurrently. After balance arrives, TokenBar waits up to five seconds for
