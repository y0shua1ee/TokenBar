...
TokenBar reads Qoder big model credit usage from the Qoder account dashboard. It supports the international
...
4. Leave Cookie source on **Automatic**, or switch to **Manual**. For `qoder.com`, paste a `Cookie:` header or a cURL/
   ...
   or `Host` header so TokenBar can select the matching site.
...
```bash
tokenbar usage --provider qoder --verbose
```
...
Sign in to Qoder in Chrome, then refresh TokenBar. If browser import is unavailable, switch to manual mode. Paste a
...
Log in to Qoder again. TokenBar clears invalid cached cookies and retries fresh browser cookies in automatic mode.
