...
TokenBar reads organization spend from ai&'s documented request-log API. ai& (aiand.com) is an OpenAI/Anthropic-compatible
...
prefix and are shown once at creation time. Add the key in TokenBar Settings → Providers → ai&.
...
TokenBar does not read ai& browser cookies, console sessions, or inference prompts; only per-request cost/currency
...
endpoint that reports cost, so TokenBar sums spend from it.
...
- The billing currency is read from the log rows themselves — TokenBar never assumes a currency. If the organization
...
- TokenBar reads at most 10 pages (1,000 requests) per refresh. If the organization has more requests in the window,
...
- A `429` means the per-minute rate limit was hit; TokenBar retries on the next refresh cycle.
