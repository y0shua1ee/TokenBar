...
shutdown, Antigravity is the replacement path for Gemini quota tracking in TokenBar. Launch
the Antigravity app or run `agy`, sign in, then refresh. See `docs/gemini.md` for the Gemini
provider migration notes. TokenBar offers the handoff only after an observed Google migration
...
run `agy` once and sign in. TokenBar keeps the signed-in `agy` local HTTPS server alive briefly
...
TokenBar relies on the `agy` CLI HTTPS source or the Google OAuth fallback. Without a signed-in
...
can take a few extra seconds while TokenBar waits for readiness; later refreshes reuse the warmed session.

The local and CLI paths both prefer Antigravity's internal `RetrieveUserQuotaSummary` quota payload and may fall back to
`GetUserStatus`, then `GetCommandModelConfigs`; TokenBar never scrapes the desktop UI or the `agy` TUI.
...
When OAuth identifies the account but quota endpoints deny access, TokenBar shows `Limits not available` instead of an
empty quota card.
...
- Removing the last saved token account that matches `~/.tokenbar/antigravity/oauth_creds.json` deletes that shared file,
  so a removed TokenBar account does not silently continue refreshing through the legacy shared cache.
...
1. **Process detection**
   ...
   - TokenBar collects all valid local app language-server candidates and probes each reachable one. If multiple
...
When source mode is `auto` or `cli` and the desktop local probe fails, TokenBar resolves `agy` via:
...
TokenBar launches `agy` in a PTY because the CLI exposes its quota server only while the interactive process is alive.
...
- Before a one-shot CLI invocation launches `agy`, TokenBar spends at most two seconds looking for an already-running,
  same-user `agy` at the selected binary path and reuses its tokenless local HTTPS endpoint when it returns parseable
  usage for the selected account. Long-lived app/server refreshes keep using TokenBar's managed session, and
  TokenBar-owned pids are excluded from external reuse so probe/idle lifecycle accounting stays balanced.
- Readiness is endpoint-based: TokenBar retries until one of the quota endpoints parses, because fresh `agy`
...
- TokenBar records the launched pid + executable identity and conservatively reaps only its own matching stale `agy`
  process on the next launch. It never blind-kills a user-launched `agy`.

### 3) Antigravity IDE local probe

When the Antigravity 2.0 app and `agy` CLI are unavailable, TokenBar probes Antigravity IDE language servers with
...
When source mode is `auto`, OAuth is used after app, `agy` CLI, and IDE paths fail if TokenBar has a selected/injected
...
- Representative selection:
  ...
  - For each group, TokenBar uses the lowest remaining known quota row and preserves that row's reset metadata.
