...
TokenBar reads the accounting data exposed by a sub2api group API key. It calls only `GET /v1/usage`; it does not
...
> may violate upstream provider terms](https://github.com/Wei-Shaw/sub2api#readme). TokenBar only reads usage from
...
label such as `Claude`, `Codex`, or `Gemini`. TokenBar's account switcher selects the active key; stacked account mode
can fetch and display several group keys at once.

## Display

The response mode determines what TokenBar shows:
...
Usage totals are scoped to the authenticated key. Wallet balance is scoped to the owning user, so TokenBar keeps it
on each account card and does not sum it across keys.

For subscription groups, TokenBar displays the daily, weekly, and monthly counters returned by `/v1/usage`. These
...
Keys are stored using TokenBar's existing provider config or token-account storage. TokenBar sends a key only to the
