...
> **Unofficial integration:** T3 Chat does not publish an API for this data. TokenBar reads a cookie-authenticated
...
TokenBar imports your browser session cookie automatically. TokenBar sends the cookie only to
`https://t3.chat`.
...
5. Paste the full `curl` command into the **T3 Chat cookie** field in TokenBar settings.
...
```bash
# Show T3 Chat usage
tokenbar usage --provider t3chat

# Or use the alias
tokenbar usage --provider t3-chat
tokenbar usage --provider t3
```

T3 Chat provides no token-cost data. The `usage --format json` output contains usage and identity
data, while `tokenbar cost --provider t3chat` is unsupported.
...
| `T3 Chat session cookie is invalid or expired` | Stale or revoked session cookie | Sign out of T3 Chat, sign back in, then refresh TokenBar or repaste the cookie |
...
| `Could not parse T3 Chat usage` | T3 Chat changed its tRPC response shape | Open a TokenBar issue with a redacted response sample |
