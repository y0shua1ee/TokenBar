...
- `KiloOrganization` (Sendable, Codable, Equatable) in `Sources/TokenBarCore/Providers/Kilo/`:
  - `id: String`
  - `name: String`
...
- Unit tests (in `Tests/TokenBarTests`):
...
- CLI snapshot test (`Tests/TokenBarTests/CLIRendererTests` or similar): `tokenbar-cli kilo` shows one section per active scope when orgs are enabled.
