...
1. **API key** stored in `~/.tokenbar/config.json` or supplied via `MOONSHOT_API_KEY` / `MOONSHOT_KEY`.
   TokenBar stores the key in config after you paste it in Settings → Providers → Moonshot / Kimi API.
2. **Region**
...
- `Sources/TokenBarCore/Providers/Moonshot/MoonshotProviderDescriptor.swift` (descriptor + fetch strategy)
- `Sources/TokenBarCore/Providers/Moonshot/MoonshotUsageFetcher.swift` (HTTP client + JSON parser)
- `Sources/TokenBarCore/Providers/Moonshot/MoonshotSettingsReader.swift` (env var resolution)
- `Sources/TokenBar/Providers/Moonshot/MoonshotProviderImplementation.swift` (settings field + activation logic)
- `Sources/TokenBar/Providers/Moonshot/MoonshotSettingsStore.swift` (SettingsStore extension)
